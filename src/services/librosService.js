...
export const librosService = {
  ...
  async getAllLibros() {
    try {
      ...
      const message =
        error.response?.data?.message || "Error al obtener libros";
  ...
  async getLibroById(id) {
    try {
      ...
      const message =
        error.response?.data?.message || "Error al obtener el libro";
      return { success: false, error: message };
    }
  },
  ...
  async searchLibros(query) {
    try {
      ...
      const filteredLibros = result.libros.filter(
        (libro) =>
